...
if(isset($_POST['submit_add']))
{
	...
	$productpriceid = $_POST['productpriceid'];
		...
		$scart->productpath="productdetail.php?id=".$productid;	
		$scart->addtocart();
...
$objrecent =  new mysql();
$objrecent->stmt = "delete from tblrecent_view where userid=".$recent_usernid." and  productid=".$_REQUEST["id"];
$objrecent->execute();
$objrecent->stmt = "insert into tblrecent_view(userid,productid,viewdate)values(".$recent_usernid.",".$_REQUEST["id"].",'".$orderdatewithtime."')";
$objrecent->execute();
...
$cat = new mysql();
$query = "select a.productid,b.catname as pro_catname,b.catid as pro_catid ,a.productname as proname,a.title as product_title from tblproduct a inner join tblcategory b on a.catid=b.catid  where a.productid='".$_REQUEST['id']."' and a.blockstatus='active' and a.bstatus=1";
$cat->stmt =$query;
$cat->execute();
...
$nextlink = '#';
$objlink = new mysql();
$query = " select productid as prelink from tblproduct where catid=".$pro_catid." and productid<".$_REQUEST['id']." and blockstatus='active' and bstatus=1 order by productid desc limit 0,1";
$objlink->stmt =$query;
$objlink->execute();
...
$objlink = new mysql();
$query = " select productid as nextlink from tblproduct where catid=".$pro_catid." and productid>".$_REQUEST['id']." and blockstatus='active' and bstatus=1 order by productid limit 0,1";
$objlink->stmt =$query;
$objlink->execute();
if($objlink_result = $objlink->fetch_array())
{
...
<?php
$meta = new mysql();
$query = "select a.metaname,b.metacontent from tblmeta_master a inner join tblmeta b on a.metaid=b.metaid where a.activestatus='active' and a.bstatus=1 and b.activestatus='active' and b.bstatus=1 and b.metatype='product' and b.typeid=".$_REQUEST['id'];
$meta->stmt = $query;
$meta->execute();
...
function showprice(proprice,saleprice,sizeid,productpriceid,showpricevalue)
{
...
function selectstock_size(divid,sizename)
{
	...
	if(document.getElementById(divid).style.border=="2px solid rgb(150, 152, 152)")
	{
		document.getElementById(divid).style.border="solid 2px #ff0000";
...
<?
$cat = new mysql();
 $query="select a.*,b.price as pro_price,b.sale_price as pro_saleprice, b.sizeid as defaultsize, b.stock ,b.color_imagename as colorimageselect,c.catname as subcatname,d.catname as catname  from tblproduct a inner join tblproduct_price b on a.default_priceid=b.nid inner join tblcategory c on a.catid=c.catid inner join tblcategory d on c.parentid=d.catid  where  a.productid='".$_REQUEST['id']."' and a.blockstatus='active' and a.bstatus=1 and b.bstatus=1 and b.blockstatus='active' and c.bstatus=1 and c.blockstatus='active' ";	
$cat->stmt =$query;
$cat->execute();
$cat_result = $cat->fetch_array();
extract($cat_result);
            ...
            <div class="col-md-12">
              <div class="product-content-right">
                <div class="product-breadcroumb">
                  <?=$catname?>
                  &nbsp; / &nbsp;<a href="products.php?catid=<?=$pro_catid?>">
                  <?=$pro_catname?>
                  </a>&nbsp; / &nbsp;
                  <?=$proname?>
                </div>
                  ...
                  <div class="col-sm-7">
                    <div class="product-images">
                      <div class="product-main-img">
                        <div class="easyzoom easyzoom--overlay easyzoom--with-thumbnails"> <a href="image/product/big/<?=$image1 ?>"> <img src="image/product/big/<?=$image1 ?>" alt="" width="100%" /> </a> </div>
                      ...
                      <div class="product-gallery" style=" background:#ffffff;">
                        <ul class="thumbnails" style="padding:0px;">
                          <?php     
										  if($image1!=""){
										  ?>
                          <li> <a href="image/product/big/<?=$image1 ?>" data-standard="image/product/big/<?=$image1 ?>"> <img src="image/product/big/<?=$image1 ?>" alt=""  /> </a> </li>
                          <?php
										  }
										  ?>
                          <?php     
										  if($image2!=""){
										  ?>
                          <li> <a href="image/product/big/<?=$image2 ?>" data-standard="image/product/big/<?=$image2 ?>"> <img src="image/product/big/<?=$image2 ?>" alt=""  /> </a> </li>
                          <?php
										  }
										  ?>
                          <?php     
										  if($image3!=""){
										  ?>
                          <li> <a href="image/product/big/<?=$image3 ?>" data-standard="image/product/big/<?=$image3 ?>"> <img src="image/product/big/<?=$image3 ?>" alt=""  /> </a> </li>
                          <?php
										  }
										  ?>
                          <?php     
										  if($image4!=""){
										  ?>
                          <li> <a href="image/product/big/<?=$image4 ?>" data-standard="image/product/big/<?=$image4 ?>"> <img src="image/product/big/<?=$image4 ?>" alt=""  /> </a> </li>
                          <?php
										  }
										  ?>
                        </ul>
                      ...
                      <div class="visible_only_desktop" style="background-color:#ffffff; border-bottom:solid 1px #666666;">
                          ...
                          <tbody>
                            <tr>
                              <td class="prev"><? if($prelink=="#") { ?>
                                ...
                                <? } else { ?>
                                <a href="productdetail.php?id=<?=$prelink?>"><i class="fa fa-angle-left" aria-hidden="true"></i> Previous</a>
                                <? } ?>
                              </td>
                              <td class="back-to"></td>
                              <td class="next"><? if($nextlink=="#") { ?>
                                ...
                                <? } else { ?>
                                <a href="productdetail.php?id=<?=$nextlink?>">Next <i class="fa fa-angle-right" aria-hidden="true"></i></a>
                                <? } ?>
                              </td>
                            </tr>
                          </tbody>
                        ...
                        <input type='hidden' name='imagename' value='<?=$glob['rootRel']?>image/product/big/<?=$image1 ?>' id='imagename'/>
                          <?php
										...
										$prosize=new mysql();
										 $sizequery = "SELECT productid as procolorid,colorname procolorname,colorimage as procolorimage FROM tblproduct a where a.productcode='".$productcode."' and a.catid=".$catid." and a.bstatus=1 and a.blockstatus='active' order by colorname";
                          ...
                          <div style="margin-bottom:22px;">
                            ...
                            <ul class="colour-boxs" style="padding:0px;">
                              <?php
											...
											?>
                              <li> <a href="productdetail.php?id=<?=$procolorid?>"> <img id="<?=$procolorname?>" onMouseOver="showcolorname(this.id);" <?php if($productid==$procolorid){ ?> style='border:solid 2px #969898;' <?php } else { ?>  style='border:solid 1px #aaaaaa;' <? }?>src="image/color/<?=$procolorimage?>" width="100%"></a></li>
                              <?php }
											 ?>
                            </ul>
                            ...
                            <?php
										...
										$prosize=new mysql();
										 $sizequery = "SELECT b.sizename,a.sizeid,a.nid as pro_priceid,a.sale_price as size_price,a.stock FROM `tblproduct_price` a inner join tblsize b on a.sizeid=b.sizeid where a.productid='".$_REQUEST['id']."' and a.bstatus=1 and a.blockstatus='active' and b.bstatus=1 and b.blockstatus='active' group by a.sizeid";
									$prosize->stmt=$sizequery;
									$prosize->execute();
                            ...
                            <ul class="size-boxes" style="padding:0px;">
							  ...
							  <?php if($defaultsize==$sizeid){ ?> style='border:solid 2px #969898; cursor:pointer;'  onClick="showprice('<?=$price?>','<?=$size_price?>','<?=$sizeid?>','<?=$pro_priceid ?>','<?=$objcurr->get_currency_amount($size_price);?>');" id="divs<?=$sizeid?>"
							  <?php } else if($stock>$stock_minvalue1) { ?>  style='border:solid 1px #aaaaaa; cursor:pointer;' onClick="showprice('<?=$price?>','<?=$size_price?>','<?=$sizeid?>','<?=$pro_priceid ?>','<?=$objcurr->get_currency_amount($size_price);?>');" id="divs<?=$sizeid?>"
                     ...
                     <div style="background: rgb(255, 255, 255) none repeat scroll 0% 0%; text-align: center; font-size: 12px; margin: 0px auto; width: 265px; padding: 15px 5px 0px; margin-bottom:40px;">
                      ...
                      <div style="float: left;">
                           ...
                           <form method="post" action="" name="frmwishlist" id="frmwishlist0">
                           	<span style="cursor:pointer; padding:5px 10px 5px 10px; " onClick="submit_wishlist(0);"> <img id="imgproduct_wishlist" src="images/wishlist_product.png" style="height:17px; margin-right:3px;margin-top: -3px;" onMouseOver="changewishlist_image(this.id,'images/wishlist1.png')"  onMouseOut="changewishlist_image(this.id,'images/wishlist_product.png')" /> Add To Wishlist </span> 
                            <input type="hidden" id="wishlistpro_id" name="wishlistpro_id" value="<?=$productid?>" >
                             </form>
                           </div><div style="float: left;"> 
                            &nbsp; &nbsp;  <span style="cursor:pointer; padding:5px 10px 5px 10px; margin-left:10px; " onClick="invitepopup('<?=$productid?>','<?=$productname?>');"> <img id="imgproduct_friend" src="images/friend_product.png" style="height:17px; margin-right:3px;margin-top: -3px;" onMouseOver="changewishlist_image(this.id,'images/friend1.png')"  onMouseOut="changewishlist_image(this.id,'images/friend_product.png')" /> Ask Friend </span> </div>
                           </div> 
                        </div>
...
<div class="related-products-wrapper" style="width:100%; margin:0px auto; margin-bottom:40px; float:left;">
  ...
  <div class="related-products-carousel" style="width:100%; margin:0px auto;">
    <?php
	  	$btag = new mysql();
							$query = "select a.productid,a.productname,a.productcode,a.productdesc,a.image1 from tblproduct a where catid in (".$related_categories.") and a.bstatus=1 and a.blockstatus='active' and productid<>".$_REQUEST["id"]." order by rand() limit 6 ";		
    ...
    <div class="single-product">
      <div class="product-f-image"> <a href="productdetail.php?id=<?=$productid?>"> <img width="320px;" src="image/product/big/<?=$image1?>" alt=""></a>
        <!--<div class="product-hover">                                
                                <a href="productdetail.php?id=<?=$productid?>" class="view-details-link"><i class="fa fa-link"></i>See details</a>
                            </div>-->
      </div>
      <h2 style="text-align:center; font-size:14px;"><a href="productdetail.php?id=<?=$productid?>">
        <?=$productname?>
        </a></h2>
...
<div class="related-products-wrapper" style="width:100%; margin:0px auto;  float:left;">
  ...
  <div class="related-products-carousel" style="width:100%; margin:0px auto;">
    <?php
	  	$btag = new mysql();
							$query = "select a.productid,a.productname,a.productcode,a.productdesc,a.image1 from tblproduct a inner join tblrecent_view b on a.productid=b.productid where b.userid=".$recent_usernid." and a.bstatus=1 and a.blockstatus='active'  order by nid desc limit 6 ";		
...
<div id="stock_dialog" class="modal" style="padding-bottom: 20px;">
  <!-- Modal content -->
  <div class="modal-content" style="width: 90%; max-width:450px; border-radius: 5px; border: solid 1px #666666; overflow:auto;"> <span class="close" onClick="closestock_box();">X</span> <br />
              	...
              	<?
					$prosize=new mysql();
									...
									extract($size_result);
										...
										{
										?>
                                        <li style='border:solid 2px #969898; cursor:pointer;' onClick="selectstock_size(this.id,<?php echo $sizename?>);" id="divstock<?=$sizeid?>">  
										  <?php echo $sizename?>
                                          </li>
                                        <?
										}
                ...
                <tr>
                 ...
                 <tr>
                	<td align="center">
                        ...
                        <input type="hidden" id="hid_stock_product" name="hid_stock_product" value="<?=$_REQUEST['id']?>" />
                    </td>
