...
if(isset($_POST["btnsave"]))
{
	...
	if(isset($_POST["drpalias_sizeid"]))
	{
		$pro_alias_sizeid =$_POST["drpalias_sizeid"];
	}
	...
	if(isset($_REQUEST["nid"]) && $_REQUEST["nid"]=="0")
	{
		...
		else
		{
			...
			$query = "insert into tblproduct_price set productid='".$_REQUEST["proid"]."',sizeid='".$pro_sizeid."',alias_sizeid='".$pro_alias_sizeid."',color_imagename='".$pro_colorid."',price='".$_POST["txtprice"]."',sale_price='".$_POST["txtsaleprice"]."',stock='".$_POST["txtstock"]."',blockstatus='".$_POST["rdstatus"]."' , bstatus=1";
	...
	else
	{
		...
		$query = "update tblproduct_price set productid='".$_REQUEST["proid"]."',sizeid='".$pro_sizeid."',alias_sizeid='".$pro_alias_sizeid."',stock='".$_POST["txtstock"]."',color_imagename='".$pro_colorid."',price='".$_POST["txtprice"]."',sale_price='".$_POST["txtsaleprice"]."',blockstatus='".$_POST["rdstatus"]."' , bstatus=1  where nid=".$_REQUEST["nid"];		
		$savereview->stmt = $query;
		$savereview->execute();
		$msg = "Price  Added Successfully";
		header("location:productprice.php?proid=".$_REQUEST['proid']);
...
$setprice = new mysql();
$query="update tblproduct set default_priceid='".$_REQUEST['did']."' where productid='".$_REQUEST['proid']."'"	;
$setprice->stmt = $query;
$setprice->execute();
?>
...
<td colspan="6" align="left">
					<span id="divsubcat">
					<select id="drpproducts" name="drpproducts" onchange="getproductprice();" class="txtBox">						
						<?php
						
							$subcat = new mysql();
							$query = "select * from tblproduct  where catid=$catid and bstatus=1 order by productname,colorname ";
							$subcat->stmt = $query;
							$subcat->execute();
							while($subcat_result =$subcat->fetch_array())
							{
								...
								if($_REQUEST["proid"]==$productid)
									echo "<option value='".$productid."' selected='true'>".$productname." - ".$colorname."</option>";
								else
									echo "<option value='".$productid."'>".$productname." - ".$colorname."</option>";	
...
<a href='productprice.php?proid=<?=$_REQUEST["proid"]?>&nid=0'>Add New Price</a>
</td>
...
$i=1;
   ...
   echo "<td><a href='productprice.php?proid=".$productid."&did=".$priceid."&action=dfprice'>Set Default Price</a> | <a href='productprice.php?proid=".$productid."&nid=".$priceid."'>Edit</a> | <a href='deleteproduct_price.php?proid=".$productid."&nid=".$priceid."' onclick='return confirmfunction();'>Delete</a></td>";
    echo "</tr>";
    $i = $i + 1;
	}
					...
					<td width="450" valign="top">
						...
						<tr >
                            ...
                            <tr>
								...
								<td>							
								<select name="drpalias_sizeid" id="drpalias_sizeid" class="txtBox"  style="width:210px" >
								<option value="0">--Select Size--</option>
								<?php
								
								
									$product = new mysql();
									$query= "SELECT sizeid,sizename FROM tblsize WHERE catid=".$maincatid." and isalias=1 and bstatus=1 and blockstatus='active'";	
									$query .=" order by sizename";									
									$product->stmt = $query;
									$product->execute();
									while($product_result =$product->fetch_array())
							...
							<tr >
								...
								<td >
                                <?php if(isset($_REQUEST["nid"]) && $_REQUEST["nid"]!=0){?>
									<input type="text" class="txtBox" name="txtprice" id="txtprice" value="<?=$price?>" />
                                    <?php }else{?>
                                    <input type="text" class="txtBox" name="txtprice" id="txtprice" />
                                    <?php }?>
								</td>
							</tr>
                            <tr >
								...
								<td >
                                 <?php if(isset($_REQUEST["nid"]) && $_REQUEST["nid"]!=0){?>
                                    ...
                                    <?php }else{?>
                                   <input type="text" class="txtBox" name="txtsaleprice" id="txtsaleprice"  />
                                    <?php }?>
							 ...
							 <tr >
								...
								<td >
                                 <?php if(isset($_REQUEST["nid"]) && $_REQUEST["nid"]!=0){?>
                                    ...
                                    <?php }else{?>
                                   <input type="text" class="txtBox" name="txtstock" id="txtstock" />
                                    <?php }?>
							...
							<tr >
                                ...
                                <?php if(isset($_REQUEST["nid"]) && $_REQUEST["nid"]!=0){?>
                                    ...
                                    <?php }else{?>
                                  <input type="radio" class='btnradio' name="rdstatus"  value='active' />Active
                                    <?php }?>
                                
									
									<span style="padding-left:42px">
                                     <?php if(isset($_REQUEST["nid"]) && $_REQUEST["nid"]!=0){?>
									 <input type="radio" class='btnradio' name="rdstatus" <?php if($blockstatus!=active) echo "checked='checked'  " ?> value='block'/>Block
                                    <?php }else{?>
                                   <input type="radio" class='btnradio' name="rdstatus"  value='block'/>Block
                                    <?php }?>
