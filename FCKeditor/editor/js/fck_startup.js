...
for (var i=0;i<aParams.length;i++)
{
	...
	var sParamValue=aParam[1];FCKURLParams[sParamName]=sParamValue;
}
var FCK=new Object();
...
FCKConfig.LoadPageConfig=function()
							{
								for (var A in this.PageConfig) 
									FCKConfig[A]=this.PageConfig[A];
							};
...
if (FCKBrowserInfo.IsGecko)
{
	function Window_OnResize()
	{
		...
		var iHeight=oCell.offsetHeight;oFrame.height=iHeight-2;};
	...
	function LoadPageConfig()
	{
		...
		if (FCKConfig.AllowQueryStringDebug&&(/fckdebug=true/i).test(window.top.location.search)) 
			FCKConfig.Debug=true;LoadStyles();
	};
	function LoadStyles()
	{
	...
	function LoadEditor()
	{
		FCKScriptLoader.OnEmpty=null;
		if (FCKLang) 
			window.document.dir=FCKLang.Dir;FCK.StartEditor();
	}
